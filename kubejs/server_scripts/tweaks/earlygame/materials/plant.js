...
itemTags((event, funcs) => {
	...
	var customPartialOrganic = [
     ...
     partialOrganic = customPartialOrganic.concat(getIdsOfTags(event, ['supplementaries:flower_box_plantable', 'forge:plant', 'minecraft:flowers', 'forge:compostable', 'biomeswevegone:flowers'])).filter((element, index, self) => self.indexOf(element) == index).filter(element => !element.includes('leaves')).filter(element => !subtractPartialOrganic.includes(element))
})
