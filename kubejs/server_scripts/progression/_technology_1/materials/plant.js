...
itemTags((event, funcs) => {
	...
	var customPartialOrganic = [
     ...
     partialOrganic = customPartialOrganic.concat(getIdsOfTags(event, ['supplementaries:flower_box_plantable', data + 'plant', 'minecraft:flowers', data + 'compostable', 'biomeswevegone:flowers'])).filter((element, index, self) => self.indexOf(element) == index).filter(element => !element.includes('leaves')).filter(element => !subtractPartialOrganic.includes(element))
})
...
commonTags((event) => {
	...
	event.add('minecraft:flowers', missingTallFlowers)
	...
	function addWholeOrganicDrops(blocks, stick) {
		blocks.forEach(block => {
			event.addBlockLootModifier(block).addAlternativesLoot(
				LootEntry.of(stick).when(c => c.randomChance(0.25).customCondition(conditionMatchTool(data + 'tools/knives'))).customFunction(setCount(countUniform(1, 2), false)),
				LootEntry.of('notreepunching:plant_fiber').when(c => c.customCondition(conditionMatchTool(data + 'tools/knives'))).customFunction(setCount(countUniform(1, 3), false)),
	...
	function addOrganicDrops(blocks, stick) {
		blocks.forEach(block => {
			event.addBlockLootModifier(block).removeLoot(block).addAlternativesLoot(
				...
				LootEntry.of(block).when(c => c.customCondition(conditionMatchTool(data + 'shears'))),

				LootEntry.of(stick).when(c => c.randomChance(0.05).customCondition(conditionMatchTool(data + 'tools/knives'))),
				LootEntry.of('notreepunching:plant_fiber').when(c => c.customCondition(conditionMatchTool(data + 'tools/knives'))).customFunction(setCount(countUniform(1, 2), false)),
