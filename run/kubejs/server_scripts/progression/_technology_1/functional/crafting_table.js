recipes((event, funcs) => {
     ...
     event.replaceInput({input: 'minecraft:crafting_table'}, 'minecraft:crafting_table', dataTag + 'workbenches')
     funcs.removeRecipes({type: 'minecraft:crafting_shaped', output: dataTag + 'workbenches'}, (result, ingredients) => {
          ...
          if (result.is('minecraft:chest'))
               ingredient = tags.non_aether_planks
          else if (result.is('aether_genesis:skyroot_crafting_table'))
               ingredient = tags.aether_planks

          funcs.toolDamagingInsert(result, [['decorative_blocks:lattice', [0, 1]], [ingredient, 2], ['#minecraft:axes', 3]])
     })

     event.shapeless('minecraft:crafting_table', dataTag + 'workbenches')
})

commonTags(event => {
     event.add(data + 'workbenches', [
